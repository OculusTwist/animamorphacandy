...
public class MessagesConfig {
	...
	private static List<MessagesConfig> configs = new ArrayList<>();

	public MessagesConfig(String n) {
		configs.add(this);
	}
	 ...
	 */
	public static MessagesConfig getConfig(String n) {
		for (MessagesConfig c : configs) {
			return c;
		}
		return new MessagesConfig(n);
	}
	...
	public boolean exists() {
		if (fc == null || file == null) {
			File temp = new File(getDataFolder(), "messages.yml");
			if (!temp.exists()) {
				return false;
			}
			file = temp;
	...
	public File getDataFolder() {
		File dir = new File(
				MessagesConfig.class.getProtectionDomain().getCodeSource().getLocation().getPath().replaceAll("%20", " "));
